...
describe('Search function', () => {
  ...
  context('Nothing fails', () => {
    ...
    describe('logs', () => {
      ...
      describe('releases', () => {
        it('1', function () {
          assert(this.logger.info.getCalls()[1].args[0].endsWith('P(1/2) I(1/2) R-1 (M-undefined) OK'));
        });

        it('2', function () {
          assert(this.logger.info.getCalls()[2].args[0].endsWith('P(1/2) I(2/2) R-2 (M-undefined) OK'));
        });

        it('3', function () {
          assert(this.logger.info.getCalls()[4].args[0].endsWith('P(2/2) I(1/3) R-3 (M-undefined) OK'));
        });

        it('4', function () {
          assert(this.logger.info.getCalls()[5].args[0].endsWith('P(2/2) I(2/3) R-4 (M-undefined) OK'));
        });
      });

      describe('search pages', () => {
        it('1', function () {
          assert(this.logger.info.getCalls()[0].args[0].endsWith('P 1/2: 2 items'));
        });

        it('2', function () {
          assert(this.logger.info.getCalls()[3].args[0].endsWith('P 2/2: 3 items'));
        });
      });
    });
    ...
    it('Logs info about release that is surely no match', function () {
      assert(this.logger.debug.getCalls()[0].args[0].endsWith('R-5 tracklist length (2) does not match the album\'s (1)'));
    });
  });
  ...
  describe('Discogs search promise rejects because of timeout', () => {
    describe('First page', () => {
      ...
      it('Error message is as expected', function () {
        assert(this.logger.error.getCalls()[0].args[0].endsWith('SEARCH P-1 TIMEOUT'));
      });
    ...
    describe('Second page', () => {
      ...
      it('Error message is as expected', function () {
        assert(this.logger.error.getCalls()[0].args[0].endsWith('SEARCH P-2 TIMEOUT'));
      });
  ...
  describe('Discogs release retrieval rejects because of timeout', () => {
    ...
    it('Error message is as expected', function () {
      assert(this.logger.error.getCalls()[0].args[0].endsWith('R-1 P-(1/2) TIMEOUT'));
    });
  ...
  describe('Discogs release retrieval rejects because of 429', () => {
    ...
    it('Error message is as expected', function () {
      assert(this.logger.error.getCalls()[0].args[0].endsWith('A 429 was thrown (too many requests). Search will pause for 0.001s'));
    });
  ...
  describe('Discogs get release rejects because of anything else', () => {
    ...
    it('Error message is as expected', function () {
      assert(this.logger.error.getCalls()[0].args[0].includes('EXCEPTION. Search removed.'));
    });
